...
export async function renderLookup(licence: Licence, outputDir?: string): Promise<void> {
  ...
  if (kind === 'key' || kind === 'both') {
    ...
    actionMsg = copied ? `✓ Key copied to clipboard` : `(clipboard not available)`
  }

  if (kind === 'file' || kind === 'both') {
    const path = await writeLicenceFile(licence.licenceFile!, outputDir)
    const fileMsg = path ? `✓ Licence file saved to ${path}` : `✗ Failed to write licence file`
...
export async function renderDisambiguate(matches: Licence[], outputDir?: string): Promise<void> {
  ...
  select.on(SelectRenderableEvents.ITEM_SELECTED, async (_index: number, option: { value: Licence }) => {
    renderer.destroy()
...
function buildLicenceCard(renderer: CliRenderer, licence: Licence): BoxRenderable {
